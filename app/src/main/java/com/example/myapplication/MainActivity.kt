...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode==RC_SIGN_IN){
            var Task = GoogleSignIn.getSignedInAccountFromIntent(data)   // revisar dependencia googlesignin
            ...
            auth.signInWithEmailAndPassword(textEmail.text.toString(), textPassword.text.toString())
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(baseContext, "Authentication faillida.",Toast.LENGTH_SHORT).show()
                        updateUI(null)
                    }
