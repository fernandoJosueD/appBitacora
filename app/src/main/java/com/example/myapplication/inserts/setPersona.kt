    ...
    fun guardarInfoUsuario() {
        ...
        if (currentUser != null) {
            ...
            pesoIMC = pesoKilogramos / alcuadrado
                ...
                if (pesoIMC <= 18.5) {
                    textViewGenero.text = "IMC rango de peso insuficiente " + "%.2f".format(pesoIMC)
            } else if (pesoIMC <= 25) {
                    textViewGenero.text = "IMC rango de peso normal " + "%.2f".format(pesoIMC)
            } else if (pesoIMC <= 30) {
                    textViewGenero.text = "IMC rango de sobrepeso " + "%.2f".format(pesoIMC)
            } else if (pesoIMC <= 40) {
                    textViewGenero.text = "IMC rango de obesidad " + "%.2f".format(pesoIMC)
            } else if (pesoIMC >= 40) {
                    textViewGenero.text = "IMC rango de Obesidad Morvida " + "%.2f".format(pesoIMC)
            }
...
//------------------------------------------------------------------------------------------------
    ...
    fun validar(): Boolean {
        var retorno = true
        val nom: String = editTextNombre.text.toString()
        val segnom: String=editTextSegundoNombre.text.toString()
        val ape: String = editTextApellido.text.toString()
        val est: String = editTextAltura.text.toString()
        val eda: String = editTextEdad.text.toString()
        val pes: String = editTextPeso.text.toString()



        if (nom.isEmpty()) {
            editTextSegundoNombre.error = "ingrese un nombre"
            retorno = false
        }
        if (segnom.isEmpty()) {
            editTextNombre.error = "ingrese segundo nombre"
            retorno = false
        }
        if (ape.isEmpty()) {
        ...
        if (est.isEmpty()) {
            editTextAltura.error = "ingrese su altura"
            retorno = false
        }
        if (eda.isEmpty()) {
            editTextEdad.error = "ingrese una edad"
            retorno = false
        }
        if (pes.isEmpty()) {
            editTextPeso.error = "ingrese su peso"
            retorno = false
        }
        return retorno
    }
